cubism_contextPrototype.opennms = function(host) {
  ...
  source.metric = function(expression) {
    ...
    var metric = context.metric(function(start, stop, step, callback) {
      ...
      d3.text(host + "/render?format=raw"
          ...
          + "&from=" + cubism_opennmsFormatDate(start - 2 * step) // off-by-two?
          + "&until=" + cubism_opennmsFormatDate(stop - 1000), function(text) {
        if (!text) return callback(new Error("unable to load data"));
        callback(null, cubism_opennmsParse(text));
      });
    }, expression += "");
...
// opennms understands seconds since UNIX epoch.
function cubism_opennmsFormatDate(time) {
  return Math.floor(time / 1000);
}

// Helper method for parsing opennms's raw format.
function cubism_opennmsParse(text) {
  var i = text.indexOf("|"),
